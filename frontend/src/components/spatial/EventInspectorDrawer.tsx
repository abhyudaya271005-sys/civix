...
const getEpistemicBadge = (status: string) => {
  switch (status) {
    ...
    case 'REFUTED':
      return {
        label: 'REFUTED',
        style: 'bg-red-950/60 text-red-300 border-red-800 line-through',
        icon: Ban
      };
    case 'INCONCLUSIVE':
    default:
...
export const EventInspectorDrawer: React.FC<EventInspectorDrawerProps> = ({
  ...
  return (
    <div className="bg-[#0b0e17] border border-[#151d2a] rounded p-4 shadow-md flex flex-col justify-between space-y-4 font-sans text-xs text-slate-100">
      <div>
        {/* Drawer Header */}
        <div className="flex items-center justify-between border-b border-[#151d2a] pb-2.5 mb-3">
          <div className="flex items-center space-x-1.5">
            <Info className="w-4 h-4 text-[#BD3535]" />
          ...
          <button
            onClick={onClose}
            className="text-slate-400 hover:text-white p-1 rounded hover:bg-[#151d2a] transition-colors"
        ...
        <div className="grid grid-cols-2 gap-2 mb-4">
          {/* Predicate Badge */}
          <div className="bg-[#0e131d] rounded border border-[#151d2a] p-2 flex flex-col justify-between">
            <span className="text-[9px] font-bold text-slate-400 uppercase tracking-wider mb-1">
              Predicate
            </span>
            <div className="flex items-center space-x-1.5">
              <PredIcon className="w-3.5 h-3.5 text-blue-400" />
        ...
        {/* Temporal Bounds */}
        <div className="bg-[#0e131d] rounded border border-[#151d2a] p-3 space-y-2 mb-4">
          <div className="flex items-center justify-between border-b border-[#151d2a] pb-1.5">
          ...
          <div className="flex items-center justify-between">
            ...
            <span className="font-mono text-[11px] font-semibold text-white">
              {props.is_open_ended ? (
                <span className="text-amber-300 bg-amber-950/60 px-1 rounded font-bold text-[10px] border border-amber-800">
                  OPEN ENDED
                </span>
              ) : (
        ...
        {props.location_predicate === 'PINGED_TOWER' && (
          <div className="bg-amber-950/40 border border-amber-900/60 text-amber-300 p-2.5 rounded text-[11px] mb-4 space-y-1">
        ...
        <div className="space-y-3">
          ...
          <div>
            ...
            {props.source_record_id ? (
              <div className="bg-[#0e131d] border border-[#151d2a] rounded p-2 flex items-center justify-between">
                <div className="flex items-center space-x-1.5">
                ...
                <span className="text-[9px] font-bold bg-blue-950/60 text-blue-300 px-1.5 py-0.5 rounded border border-blue-800">
                  SOURCE RECORD
                </span>
              </div>
            ) : (
              <div className="text-[11px] text-slate-500 italic bg-[#0e131d] border border-[#151d2a] p-2 rounded">
          ...
          <div>
            ...
            <div className="bg-[#0e131d] border border-[#151d2a] rounded p-2 text-[10px] font-mono space-y-1">
              <div className="flex justify-between">
                <span className="text-slate-400">Generator Version:</span>
                <span className="font-semibold text-slate-200">{props.generation_origin || '1.0.0-phase11d'}</span>
              </div>
              <div className="flex justify-between pt-1 border-t border-[#151d2a]">
                <span className="text-slate-400">Generation Run ID:</span>
                <span className="text-slate-400">{props.generation_run_id ? `${props.generation_run_id.slice(0, 8)}...` : 'N/A'}</span>
              </div>
            </div>
          </div>
        </div>
